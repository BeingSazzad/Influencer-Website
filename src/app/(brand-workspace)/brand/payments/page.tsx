            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="border-b border-[#E7E7E2] text-[#73736A] font-extrabold uppercase tracking-wider">
                  ...
                  <th className="pb-3 px-4">Campaign & Recipient</th>
                  ...
                  return (
                    <tr key={txn.id} className="hover:bg-[#FAFAF8] transition-colors">
                      ...
                      <td className="py-4 px-4 font-mono font-bold text-[#0A0A0A]">
                        <div>{txn.id}</div>
                        <div className="text-[11px] font-sans text-[#73736A] font-normal">{txn.date}</div>
                      </td>

                      {/* Campaign & Creator */}
                      <td className="py-4 px-4">
                        <div className="font-bold text-[#0A0A0A]">{txn.creatorName}</div>
                        <div className="text-[11px] text-[#73736A] truncate max-w-xs">{txn.campaignTitle}</div>
                      </td>

                      {/* Type Badge */}
                      <td className="py-4 px-4">
                        {isTopUp ? (
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-[#EEF7F2] text-[#23744D] font-bold text-[11px]">
                            <ArrowDownLeft className="w-3 h-3" /> Wallet Deposit
                          </span>
                        ) : isEscrow ? (
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-[#FFF0F5] text-[#FF2D78] font-bold text-[11px]">
                            <Lock className="w-3 h-3" /> In Escrow
                          </span>
                        ) : isRelease ? (
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-[#F4F4F0] text-[#0A0A0A] font-bold text-[11px]">
                            <ArrowUpRight className="w-3 h-3" /> Escrow Released
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-[#F4F4F0] text-[#73736A] font-bold text-[11px]">
                            Platform Fee (15%)
                          </span>
                        )}
                      </td>
